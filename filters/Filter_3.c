...
*/
unsigned char* covert_to_mono ( unsigned char *data_buffer , int file_size , INFO_HEADER bmp_info_header  , COLOR_PALLET *bgr_pallete)
{
   int i = 0 , j = 0 , k = 0 , padding = 0 ;
   padding = 4 - ( ( bmp_info_header.image_width * 3 ) % 4 );
   unsigned char *converted_buffer = calloc ( file_size/3 , sizeof(char));
   ...
   if (padding == 4)
   for ( i = 0 , j = 0 ; i <= file_size ; i += 3 , j ++)
   {
      //*(converted_buffer + j) = ( *(data_buffer + i) *.25 ) +  (*(data_buffer + i + 1) * .65 ) +  (*(data_buffer + i + 2) * .10) ;
      *(converted_buffer + j) = rgb24_to_rgb8 (  *(data_buffer + 3)  ,  *(data_buffer + i + 1)  ,  *(data_buffer + i + 3)  , bgr_pallete);
   ...
   else
   {
      ...
      *(converted_buffer ) = ( *(data_buffer ) *.20 ) +  (*(data_buffer + 1) * .70 ) +  (*(data_buffer + 2) * .10) ;
      ...
      {
         *(converted_buffer + j) = rgb24_to_rgb8 (  *(data_buffer + i)  ,  *(data_buffer + i + 1)  ,  *(data_buffer + i + 2)  , bgr_pallete);
...
*/
unsigned char rgb24_to_rgb8 (unsigned char ablue, unsigned char agreen , unsigned char ared , COLOR_PALLET *bgr_pallete)
{
   unsigned char blue = ( ablue * 2 ) / 255;
   unsigned char green = ( agreen * 2 ) / 255;
   unsigned char red = ( ared * 2 ) / 255;
   ( bgr_pallete + ( blue << 6 | green << 4 | red << 2 ) )->blue     = ablue ;
   ( bgr_pallete + ( blue << 6 | green << 4 | red << 2 ))->green    = agreen;
   ( bgr_pallete + ( blue << 6 | green << 4 | red << 2 ) )->red      = ared ;
   ( bgr_pallete + ( blue << 6 | green << 4 | red << 2 ) )->unused   = '\0';

   return ( blue << 6 | green << 4 | red << 2 ) ;
}
